package com.han.common.storage.core.s3;
 ...
 */
public class S3StorageClient extends AbstractStorageClient<S3StorageClientConfig> {
    ...
    public S3StorageClient(Long id, S3StorageClientConfig config) {
        super(id, config);
    }
    ...
    private String resolveRegion() {
        ...
        if (host.contains(S3StorageClientConfig.ENDPOINT_ALIYUN)) {
            // 匹配 oss-{region}.aliyuncs.com 格式
            if (host.startsWith("oss-") && host.contains("." + S3StorageClientConfig.ENDPOINT_ALIYUN)) {
                String regionPart = host.substring(4, host.indexOf("." + S3StorageClientConfig.ENDPOINT_ALIYUN));
        ...
        if (host.contains(S3StorageClientConfig.ENDPOINT_TENCENT)) {
            // 匹配 cos.{region}.myqcloud.com 格式
            if (host.startsWith("cos.") && host.contains("." + S3StorageClientConfig.ENDPOINT_TENCENT)) {
                String regionPart = host.substring(4, host.indexOf("." + S3StorageClientConfig.ENDPOINT_TENCENT));
                if (StrUtil.isNotEmpty(regionPart)) {
                    return regionPart;
                }
            }
